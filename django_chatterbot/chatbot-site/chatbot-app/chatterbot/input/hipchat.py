...
class HipChat(InputAdapter):
	...
	def get_initial_ids(self):
		...
		"""
		data = self.view_recent_room_history(self.hipchat_room , max_results=75)
	...
	def view_recent_room_history(self , room_id_or_name , max_results=1):
		"""
		https://www.hipchat.com/sphinx/apiv2/method/view_recent_room_history
		"""
		import requests
		
		recent_histroy_url = '{}/v2/room/{}/history?max-results={}'.format(self.hipchat_host , room_id_or_name ,
			max_results)
		
		response = requests.get(recent_histroy_url , headers=self.headers)
