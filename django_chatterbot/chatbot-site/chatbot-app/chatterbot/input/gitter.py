...
class Gitter(InputAdapter):
	...
	def join_room(self , room_name):
		...
		response = requests.post(endpoint , headers=self.headers , json={'uri': room_name})
		self.logger.info('{} joining room {}'.format(response.status_code , endpoint))
	...
	def get_user_data(self):
		...
		self.logger.info('{} retrieving user data {}'.format(response.status_code , endpoint))
		self._validate_status_code(response)
		return response.json()
	...
	def mark_messages_as_read(self , message_ids):
		...
		response = requests.post(endpoint , headers=self.headers , json={'chat': message_ids})
		self.logger.info('{} marking messages as read {}'.format(response.status_code , endpoint))
		self._validate_status_code(response)
		return response.json()
	...
	def get_most_recent_message(self):
		...
		response = requests.get(endpoint , headers=self.headers)
		self.logger.info('{} getting most recent message'.format(response.status_code))
		self._validate_status_code(response)
		data = response.json()
		if data:
			return data[ 0 ]
