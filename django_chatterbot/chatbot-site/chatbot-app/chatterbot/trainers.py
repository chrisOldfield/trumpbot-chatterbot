...
class Trainer(object):
	...
	class TrainerInitializationException(Exception):
		...
		def __init__(self , value=None):
			default = (
					'A training class must be specified before calling train(). ' + 'See http://chatterbot.readthedocs.io/en/stable/training.html')
	...
	def train(self , conversation):
		...
		for text in conversation:
			...
			if statement_history:
				statement.add_response(Response(statement_history[ -1 ].text))
...
def get_words_from_tweets(tweets):
	...
	for tweet in tweets:
		...
		cleaned_text = ''.join([ i if ord(i) < 128 else ' ' for i in tweet.text ])
		tweet_words = cleaned_text.split()
		
		for word in tweet_words:
			# If the word contains only letters with a length from 4 to 9
			if not (not word.isalpha() or not (3 < len(word) <= 9)):
				words.add(word)
...
class TwitterTrainer(Trainer):
	...
	def __init__(self , storage , **kwargs):
		...
		self.api = TwitterApi(consumer_key=kwargs.get('twitter_consumer_key') ,
...
class UbuntuCorpusTrainer(Trainer):
	...
	def __init__(self , storage , **kwargs):
		...
		self.data_download_url = kwargs.get('ubuntu_corpus_data_download_url' ,
			'http://cs.mcgill.ca/~jpineau/datasets/ubuntu-corpus-1.0/ubuntu_dialogs.tgz')
		
		self.data_directory = kwargs.get('ubuntu_corpus_data_directory' , './data/')
	...
	def extract(self , file_path):
		...
		extracted_file_directory = os.path.join(self.data_directory , dir_name)
...
safe_extract(tar, path=self.data_directory, members=track_progress(tar))
	...
	def train(self):
		...
		extracted_corpus_path = os.path.join(self.data_directory ,
			os.path.split(corpus_download_path)[ -1 ].split('.')[ 0 ] , '**' , '*.tsv')
