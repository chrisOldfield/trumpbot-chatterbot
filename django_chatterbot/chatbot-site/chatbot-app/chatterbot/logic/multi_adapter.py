		...
		for adapter in self.get_adapters():
			if adapter.can_process(statement):
				...
				if type(output) == tuple:
					warnings.warn('{} returned two values when just a Statement object was expected. '
				...
				self.logger.info(
					'{} selected "{}" as a response with a confidence of {}'.format(adapter.class_name , output.text ,
						output.confidence))
