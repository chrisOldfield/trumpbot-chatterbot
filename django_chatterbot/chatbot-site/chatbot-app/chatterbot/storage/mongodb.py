...
class MongoDatabaseAdapter(StorageAdapter):
	...
	def find(self , statement_text):
		...
		values[ 'in_response_to' ] = self.deserialize_responses(values.get('in_response_to' , [ ]))
	...
	def mongo_to_object(self , statement_data):
		...
		statement_data[ 'in_response_to' ] = self.deserialize_responses(statement_data.get('in_response_to' , [ ]))
	...
	def filter(self , **kwargs):
		...
		if 'in_response_to__contains' in kwargs:
			query = query.statement_response_list_contains(kwargs[ 'in_response_to__contains' ])
	...
	def update(self , statement):
		...
		update_operation = UpdateOne({'text': statement.text} , {'$set': data} , upsert=True)
		...
		for response_dict in data.get('in_response_to' , [ ]):
			...
			update_operation = UpdateOne({'text': response_text} , {'$set': response_dict} , upsert=True)
			operations.append(update_operation)
	...
	def get_response_statements(self):
		...
		_statement_query = {'text': {'$in': response_query}}
