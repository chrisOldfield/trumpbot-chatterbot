...
class DjangoStorageAdapter(StorageAdapter):
	...
	def update(self , statement):
		...
		for _response_statement in response_statement_cache:
			...
			response_statement , created = Statement.objects.get_or_create(text=_response_statement.text)
			response_statement.extra_data = getattr(_response_statement , 'extra_data' , '')
			response_statement.save()
			
			response , created = statement.in_response.get_or_create(statement=statement , response=response_statement)
	...
	def remove(self , statement_text):
		...
		responses = Response.objects.filter(Q(statement__text=statement_text) | Q(response__text=statement_text))
