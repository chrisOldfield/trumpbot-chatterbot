...
class Mailgun(OutputAdapter):
	...
	def send_message(self , subject , text , from_address , recipients):
		...
		* recipients: A list of recipient email addresses.
		...
		return requests.post(self.endpoint , auth=('api' , self.api_key) ,
			data={'from': '%s <%s>' % (self.name , from_address) , 'to': recipients , 'subject': subject ,
				'text':   text})
	...
	def process_response(self , statement , session_id=None):
		...
		self.send_message(subject , statement.text , self.from_address , self.recipients)
