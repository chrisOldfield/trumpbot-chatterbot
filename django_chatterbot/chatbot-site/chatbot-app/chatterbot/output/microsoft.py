...
class Microsoft(OutputAdapter):
	...
	def __init__(self , **kwargs):
		...
		self.headers = {'Authorization': authorization_header , 'Content-Type': 'application/json'}
	...
	def get_most_recent_message(self):
		...
		"""
		import requests
		endpoint = '{host}/api/conversations/{id}/messages'.format(host=self.directline_host , id=self.conversation_id)
		
		response = requests.get(endpoint , headers=self.headers , verify=False)
		
		self.logger.info('{} retrieving most recent messages {}'.format(response.status_code , endpoint))
	...
	def send_message(self , conversation_id , message):
		...
		https://www.hipchat.com/sphinx/apiv2/method/send_message
		"""
		import requests
		
		message_url = "{host}/api/conversations/{conversationId}/messages".format(host=self.directline_host ,
			conversationId=conversation_id)
		
		response = requests.post(message_url , headers=self.headers , data=json.dumps({'message': message}))
		
		self.logger.info('{} sending message {}'.format(response.status_code , message_url))
		self._validate_status_code(response)
