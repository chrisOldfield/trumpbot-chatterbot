...
class Gitter(OutputAdapter):
	...
	def __init__(self , **kwargs):
		...
		self.headers = {'Authorization': authorization_header , 'Content-Type': 'application/json; charset=utf-8' ,
	...
	def join_room(self , room_name):
		...
		self.logger.info('{} status joining room {}'.format(response.status_code , endpoint))
	...
	def send_message(self , text):
		...
		response = requests.post(endpoint , headers=self.headers , json={'text': text})
		self.logger.info('{} sending message to {}'.format(response.status_code , endpoint))
		self._validate_status_code(response)
		return response.json()
